  ...
  return (
    <div className="container mx-auto px-6">
      <header className="flex justify-between items-center mb-10">
        <h2 className="text-3xl md:text-4xl font-bold text-gray-900">Rekomendasi Mobil Terbaik</h2>
        <Link
      ...
      {usedCars.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {usedCars.map((item: any) => (
            <div key={item.id} className="bg-white rounded-xl overflow-hidden shadow-lg transform hover:-translate-y-2 transition-transform duration-300 group border border-gray-200">
              <div className="relative">
                <Image width={500} height={500} className="w-full h-56 object-cover" src={item.image} alt={`${item.make} ${item.model} ${item.year}`} loading="lazy" />
                <span className="absolute top-3 right-3 text-white text-xs font-bold py-1 px-3 rounded-full bg-green-600">{item.condition}</span>
              </div>
              <div className="p-6">
                ...
                <div className="flex items-center mb-2">
                  <span className="text-md text-gray-700">Mileage:</span>
                  <span className="ml-2 text-gray-900">{item.mileage}</span>
                </div>
          ...
          <div className="col-span-1 sm:col-span-2 lg:col-span-3 xl:col-span-4 flex">
            <div className="flex flex-col items-center justify-center w-full py-16 px-6">
              ...
              <div className="text-center max-w-md">
                ...
                <p className="text-gray-700 leading-relaxed mb-6">Saat ini tidak ada mobil bekas yang tersedia. Silakan cek kembali nanti atau hubungi kami untuk informasi lebih lanjut.</p>
